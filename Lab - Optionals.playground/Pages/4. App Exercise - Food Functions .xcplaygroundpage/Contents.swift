...
func mealRecall(mealType: String) -> Meal? {
    guard mealType == "Breakfast" || mealType == "Lunch" || mealType == "Dinner" else {
        return nil
    }
    return meals["\(mealType)"]
}
mealRecall(mealType: "Dinner")
mealRecall(mealType: "Breakfast")
/*:
...
func getMeals() -> [String: Meal] {
    guard let output = UserDefaults.standard.dictionary(forKey: "mealLog") as? [String: Meal] else {
        return [:]
    }
    return output
}
