...
// no if the input has a mistype like my example above, the function will return nil.
...
func inStockPrice(item: String) -> Double? {
    guard let itemStock = stock[item], itemStock > 0 else {
        return nil
    }
    return prices[item]
}

inStockPrice(item: "Donuts")
inStockPrice(item: "Chips")
/*:
